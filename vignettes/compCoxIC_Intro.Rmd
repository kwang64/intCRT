...
This vignette also shows how to use the package’s data generation utilities in order to create simulated datasets that mimics the Botswana Combination Prevention Project (BCPP) structure, using included data generation utilities. +reference
...
\[
\lambda(t \mid X(t), Z) = \lambda_0(t; Z) \exp\{\beta^T X(t)\}
\]
...
head(data)
```
++ model of `gen_time_dependent_cov()`

In the example above, we simulate a clustered interval-censored dataset using `gen_time_dependent_cov()`. The simulation setup includes:

- 200 clusters (M = 200), representing distinct communities.

- Cluster sizes ranging from 20 to 30 individuals.

- No additional random censoring (cens = 0).

- A within-cluster dependence parameter (ρ = 0.5), reflecting moderate correlation between individuals within the same cluster.

-  A stratification variable with four levels

- Baseline hazard functions specified through baseline, which define the stratum-specific survival distributions.

The simulated dataset includes left and right endpoints for interval-censored event times as well as relevant covariate and clustering information.
