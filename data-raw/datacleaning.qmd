...
yr3[yr3$de_subj_idC=="33589A", "survey"] <- "AHS-2"
```
...
yr3_AHS1 <- yr3_incident[, cols_AHS1, drop = FALSE]

AHS1_df <- bind_rows(
  yr2_AHS1 |> filter(survey == "AHS-1", de_subj_idC %in% id_valid),
  yr3_AHS1 |> filter(survey == "AHS-1", de_subj_idC %in% id_valid)) # there are participants who complete the survey 1, but skipped survey 2 and directly entered survey 3; so for AHS1 dataframe, we would love to include the survey 3 participants as well. 
